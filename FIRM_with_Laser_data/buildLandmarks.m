function Lmk = buildLandmarks(obsCorners)

% BUILDLANDMARKS  Create Lmk structure array.
%   Lmk = BUILDANDMARKS(Landmark) creates the structure array Lmk() to be
...
for lmk = 1:obsCorners

    Lmk(lmk).lmk  = lmk;
    Lmk(lmk).id   = [];
    Lmk(lmk).type = '';
    Lmk(lmk).parentObstacle = '';
    Lmk(lmk).used = false;
