...
%                 myaa_Ali('FIRM_nodes_figure') % for producing the FIRM nodes figure for a paper
                ...
                goal_node_ind = 1;
%                 text_height = 3;
%                 text(obj.FIRM_graph.PRM.nodes(start_node_ind).val(1),obj.FIRM_graph.PRM.nodes(start_node_ind).val(2),text_height,'start','color','r','fontsize',14); % we write "start" next to the goal node
%                 text(obj.FIRM_graph.PRM.nodes(goal_node_ind).val(1),obj.FIRM_graph.PRM.nodes(goal_node_ind).val(2),text_height,'goal','color','r','fontsize',14); % we write "goal" next to the goal node
