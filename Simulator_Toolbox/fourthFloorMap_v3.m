...
id = 163;
origin_to_L163 = origin_to_L136 + L_size/2 + 17 + L_size/2;
L(:,end+1) = [id; x+origin_to_L163; y; pi/2];
...
id = 164;
origin_to_L164 = origin_to_L92 + L_size/2 + 26 + L_size/2;
L(:,end+1) = [id; x+origin_to_L164; y; pi/2];
...
%% vertices
From300ToTrashCan = L_size/2 + 121;
HypothCrossToPlasticTrashCan = HypothCrossTo300 + From300ToTrashCan;
x = v_out(1,end);
y = v_out(2,end);
v_out(:,end+1) = [x - HypothCrossToPlasticTrashCan  ; y];

PointOnTrashCanToEdgeofTrashCan = 6;
x = v_out(1,end);
y = v_out(2,end);
v_out(:,end+1) = [x  ; y - PointOnTrashCanToEdgeofTrashCan];
...
id = 162;
EdgeOfTrashCanTo162 = 5.8 + L_size/2;
L(:,end+1) = [id; x - EdgeOfTrashCanTo162; y ; -pi/2];

%% vertices
EdgeOfTrashCanToMetalCornerInnerDoor = 2 + L_size/2 + EdgeOfTrashCanTo162;
x = v_out(1,end);
y = v_out(2,end);
v_out(:,end+1) = [x - EdgeOfTrashCanToMetalCornerInnerDoor ; y ];
...
%% vertices
ToSomeWhereAfterRestRoom = 750;
x = v_out(1,end);
y = v_out(2,end);
v_out(:,end+1) = [x  ; y + ToSomeWhereAfterRestRoom ];

ToSomeWhereRightofElev = 1800;
x = v_out(1,end);
y = v_out(2,end);
v_out(:,end+1) = [x + ToSomeWhereRightofElev ; y  ];

ToSomeWhereBehindDigital = 2500;
x = v_out(1,end);
y = v_out(2,end);
...
id = 165;
CornerTo165 = CornerTo60 + L_size/2 + 22 + L_size/2;
L(:,end+1) = [id; x + CornerTo165 ; y ; -pi/2];
...
id = 166;
CornerTo166 = CornerTo66 + L_size/2 + 21 + L_size/2;
L(:,end+1) = [id; x + CornerTo166 ; y ; -pi/2];
...
id = 160;
CrossTo160 = CrossTo91 + L_size/2 + 4.5 + L_size/2;
L(:,end+1) = [id; x - CrossTo160 ; y ; pi/2];

id = 161;
CrossTo161 = CrossTo160 + L_size/2 + 42.5 + L_size/2;
L(:,end+1) = [id; x - CrossTo161 ; y ; pi/2];
...
id = 157;
CrossTo157 = L_size/2;
L(:,end+1) = [id; x - CrossTo157 ; y ; pi/2];
...
id = 158;
EdgeOf_innerDoorTo158 = 35.5 - L_size/2;
L(:,end+1) = [id; x + EdgeOf_innerDoorTo158 ; y ; pi/2];
...
id = 159;
EdgeofFrontDoorTo159 = 35.5 - L_size/2;
L(:,end+1) = [id; x - EdgeofFrontDoorTo159 ; y ; pi/2];
...
%% make obj file
all_vert_meter = {v_out'*0.0254, v_AliOffice'*0.0254};
generate_obj_file_from_vertices(all_vert_meter, 'FourthFloorJune11.obj')
