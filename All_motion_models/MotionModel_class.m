...
%             disp('Solving...');
        ...
        function orbit = generate_orbit(orbit_center)
            ...
            if T_rational-floor(T_rational)==0
                V_p = MotionModel_class.Min_Velocity * ones(1,T); % we traverse the orbit with minimum linear velocity
                omega_p = MotionModel_class.Max_Yaw_Rate * ones(1,T); % we traverse the orbit with maximum angular velocity
            else
