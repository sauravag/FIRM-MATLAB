function [success , html_source] = helpwin_modified(topic,pagetitle,helptitle,varargin)
...
%   $Revision: 1.1.8.22 $ $Date: 2009/07/27 20:17:28 $
...
errormsg = javachk('mwt', 'The helpwin command');
if ~isempty(errormsg)
...
if isempty(html) && nargout
    ...
    if nargout == 2
        % Do nothing! Ali has added this "if statement". So, if the html code is requested
        % as an output there is no need to show the html in Matlab's help
        % browser.
    else
...
html_source = html;  % Ali has added this, so that he has access to the source code of generated html by "helpwin"
