classdef LaserRangeFinder_Landmarks_Range_bearing < ObservationModel_interface
    ...
    properties (Constant = true) % Note that you cannot change the order of the definition of properties in this class due to its structure (due to dependency between properties.)
        tmp_prop = LaserRangeFinder_Landmarks_Range_bearing.costant_property_constructor();  % I use this technique to initialize the costant properties in run-time. If I can find a better way to do it, I will update it, as it seems a little bit strange.
        landmarks = LaserRangeFinder_Landmarks_Range_bearing.tmp_prop.landmarks;
        obsDim = LaserRangeFinder_Landmarks_Range_bearing.tmp_prop.obsDim;
        obsNoiseDim = LaserRangeFinder_Landmarks_Range_bearing.obsDim; % observation noise dimension. In some other observation models the noise dimension may be different from the observation dimension.
        zeroNoise = zeros(LaserRangeFinder_Landmarks_Range_bearing.obsNoiseDim,1); % zero observation noise
    ...
    methods (Static = true)
        function temporary_props = costant_property_constructor()
            ...
            if Man_L == 0
                ...
                temporary_props = LaserRangeFinder_Landmarks_Range_bearing.request_landmarks();
            end
        ...
        function temporary_props = request_landmarks()
            old_prop = LaserRangeFinder_Landmarks_Range_bearing.set_figure();
            i=0;
            ...
            temporary_props.obsDim = 2*size(Landmarks,2);
            LaserRangeFinder_Landmarks_Range_bearing.reset_figure(old_prop);
            title([])
        end
        function z = h_func(x,v)
            L=LaserRangeFinder_Landmarks_Range_bearing.landmarks;
            od = LaserRangeFinder_Landmarks_Range_bearing.obsDim;
            N_L=size(L,2);
...
%                 end
        ...
        function M = dh_dv_func(x,v) %#ok<INUSD>
            ...
            M = eye(LaserRangeFinder_Landmarks_Range_bearing.obsDim);
        end
        function V = generate_observation_noise(x)
            L = LaserRangeFinder_Landmarks_Range_bearing.landmarks;
            eta = LaserRangeFinder_Landmarks_Range_bearing.eta; %#ok<PROP>
            sigma_b = LaserRangeFinder_Landmarks_Range_bearing.sigma_b;%#ok<PROP>
            obsDim = LaserRangeFinder_Landmarks_Range_bearing.obsDim;%#ok<PROP>
        ...
        function innov = compute_innovation(Xprd,Zg)
            V = zeros(LaserRangeFinder_Landmarks_Range_bearing.obsNoiseDim,1);
            Zprd = LaserRangeFinder_Landmarks_Range_bearing.h_func(Xprd,V);
            innov = Zg - Zprd;
