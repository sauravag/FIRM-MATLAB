classdef Dyn_manipulator_wall_sensing_deadzone < ObservationModel_interface
    ...
    properties (Constant = true) % Note that you cannot change the order of the definition of properties in this class due to its ugly structure!! (due to dependency between properties.)
        tmp_prop = Dyn_manipulator_wall_sensing_deadzone.costant_property_constructor();  % This property is not needed!! I only added it because I could not find any other way to initialize the rest of constant properties.
        L = [-1.5;0];
        obsDim = state.num_revolute_joints;
        obsNoiseDim =state.num_revolute_joints;
        eta = 0.1;
        sigma_b = 10^(-4);
        thresh = 1;
    ...
    methods (Static = true)
        ...
        function z = h_func(x,v)
            n = Dyn_manipulator_wall_sensing_deadzone.obsDim;
            z = x(1:n) + v;
        end
        ...
        function V = generate_observation_noise(x)
            n = Dyn_manipulator_wall_sensing_deadzone.obsDim;
            R = Dyn_manipulator_wall_sensing_deadzone.noise_covariance(x);
            V = randn(n,1).*diag(R.^(1/2));
        end
        function R = noise_covariance(x)
            ...
            for i = 1:n
                norm_d = norm(x_state.joint_2D_locations(1,i) - Dyn_manipulator_wall_sensing_deadzone.L(1));
                if norm_d < Dyn_manipulator_wall_sensing_deadzone.thresh
                    std_vector(i) = Dyn_manipulator_wall_sensing_deadzone.visible_region_std;
                else
                    std_vector(i) = Dyn_manipulator_wall_sensing_deadzone.eta *norm_d  + Dyn_manipulator_wall_sensing_deadzone.sigma_b;
                end
            end
        ...
        function innov = compute_innovation(Xprd,Zg)
            V = zeros(Dyn_manipulator_wall_sensing_deadzone.obsNoiseDim,1);
            Zprd = Dyn_manipulator_wall_sensing_deadzone.h_func(Xprd,V);
            innov = Zg - Zprd;
    ...
    methods
        function obj = draw(obj)
            ...
            % least to show
            old_prop = Dyn_manipulator_wall_sensing_deadzone.set_figure();
            ...
            eta_local = Dyn_manipulator_wall_sensing_deadzone.eta;
            L_local = Dyn_manipulator_wall_sensing_deadzone.L(1);
            sigma_b_local = Dyn_manipulator_wall_sensing_deadzone.sigma_b;
            z_mesh = zeros(size(x_mesh,1),size(x_mesh,2));
            for i = 1:size(x_mesh,1)
                for j = 1:size(x_mesh,2)
                    norm_d = norm([x_mesh(i,j)] - L_local);
                    if norm_d < 0.1;%Dyn_manipulator_wall_sensing_deadzone.thresh
                        z_mesh(i,j) = (Dyn_manipulator_wall_sensing_deadzone.visible_region_std)^2;
                    else
            ...
            colormap(cmap);
%             handle_of_plot = [handle_of_plot , plot(Dyn_manipulator_wall_sensing_deadzone.L(1,:),Dyn_manipulator_wall_sensing_deadzone.L(2,:),'kp','markerfacecolor','k','markersize',12)];
            shading interp
            Dyn_manipulator_wall_sensing_deadzone.reset_figure(old_prop);
        end
