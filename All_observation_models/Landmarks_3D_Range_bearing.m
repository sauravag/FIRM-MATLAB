...
classdef Landmarks_3D_Range_bearing < ObservationModel_interface
    ...
    properties (Constant = true) % Note that you cannot change the order of the definition of properties in this class due to its structure (due to dependency between properties.)
        ...
        eta = [0.1;0.01;0.01];%user_data_class.par.observation_model_parameters.eta; 
    ...
    methods (Static = true)
        ...
        function temporary_props = request_landmarks()
            ...
            temporary_props.obsDim = 3*size(Landmarks,2);
            Landmarks_3D_Range_bearing.reset_figure(old_prop);
            title([])
        end
        
        function z = h_func(x,v)
            L=Landmarks_3D_Range_bearing.landmarks;
            od = Landmarks_3D_Range_bearing.obsDim;
            N_L=size(L,2);
        ...
        function V = generate_observation_noise(x)
            L = Landmarks_3D_Range_bearing.landmarks;
            eta = Landmarks_3D_Range_bearing.eta; %#ok<PROP>
            sigma_b = Landmarks_3D_Range_bearing.sigma_b;%#ok<PROP>
            obsDim = Landmarks_3D_Range_bearing.obsDim;%#ok<PROP>
            N_L = size(L,2);
