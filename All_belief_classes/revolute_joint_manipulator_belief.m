classdef revolute_joint_manipulator_belief < Gaussian_belief_interface
    % This class encapsulates the Gaussian belief concept.

    methods
        function obj = revolute_joint_manipulator_belief(varargin)
        ...
        function obj = draw(obj, varargin)
            ...
            if ~isempty(ellipse_spec)
                ...
                Jacobian = nan(2,state.dim);
                sum_theta = sum(obj.est_mean.val(:));
                Jacobian(:,state.dim) = [-sin(sum_theta);cos(sum_theta)];
                for k = state.dim-1:-1:1
                    sum_theta = sum(obj.est_mean.val(1:k));
                    Jacobian(:,k) = [-sin(sum_theta);cos(sum_theta)]+Jacobian(:,k+1);
                end
                tip_cov = Jacobian * obj.est_cov * Jacobian';
